...
public:
    ...
    // Leetcode solution
    // Bruteforce Onlogn time On space
    ...
    // Compare one by one head of k linked lists and extend final sorted linked list
    // Okn time On space

    // Priority Queue Onlogk time On space
    ...
    // merge lists one by one Onk time O1 space

    // merge with divide and conquer Onlogk time O1 space
    // class Solution(object):
    // def mergeKLists(self, lists):
    //     """
    //     :type lists: List[ListNode]
    ...
    //         for i in range(0, amount - interval, interval * 2): // last parameter step constant
